...
from deepvo.helper import normalize_angle_delta, R_to_angle
...
def get_data_info(folder_list, seq_len_range, overlap, sample_times=1, pad_y=False, shuffle=False, sort=True):
    ...
    for folder in folder_list:
        start_t = time.time()
        #poses = np.load('{}{}.txt'.format(par.pose_dir, folder))  # (n_images, 6)
        
        #fpaths = glob.glob('{}{}/image_3/*.png'.format(par.image_dir, folder)) # COLOR KITTI
        fpaths = glob.glob('{}{}/image_1/*.png'.format(par.image_dir, folder)) # GRAY KITTI
        
        fpaths.sort()
        fn = '{}{}.txt'.format(par.pose_dir, folder)
        with open(fn) as f:
            lines = [line.split('\n')[0] for line in f.readlines()] 
            #poses = np.array([ R_to_angle([float(value) for value in l.split(' ')]) for l in lines])[:,:6]  # list of pose (pose=list of 12 floats)
            poses = np.array([ R_to_angle([float(value) for value in l.split(' ')]) for l in lines])
...
def get_partition_data_info(partition, folder_list, seq_len_range, overlap, sample_times=1, pad_y=False, shuffle=False, sort=True):
    ...
    for part in range(2):
        for folder in folder_list:
            ...
            with open(fn) as f:
                lines = [line.split('\n')[0] for line in f.readlines()] 
                poses = np.array([ R_to_angle([float(value) for value in l.split(' ')]) for l in lines])  # list of pose (pose=list of 12 floats)
...
class ImageSequenceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for img_path in image_path_sequence:
            ...
            img_as_img = img_as_img.convert('RGB') # comment out if using COLOR KITTI
